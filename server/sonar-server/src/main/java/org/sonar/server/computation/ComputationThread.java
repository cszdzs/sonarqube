...
public class ComputationThread implements Runnable {
  ...
  public ComputationThread(ReportQueue queue, ComponentContainer sqContainer, CEContainerFactory containerFactory) {
    this.queue = queue;
  ...
  public void run() {
    ...
    CEContainer ceContainer = containerFactory.create(sqContainer, item);
    try {
      ceContainer.process();
    } catch (Throwable e) {
