...
public class ComputationThreadLauncherTest {
  ...
  CEContainerFactory ceContainerFactory;

  @Before
  public void before() {
    this.queue = mock(ReportQueue.class);
    this.componentContainer = mock(ComponentContainer.class);
    this.ceContainerFactory = mock(CEContainerFactory.class);
  }
  ...
  public void call_findAndBook_when_launching_a_recurrent_task() throws Exception {
    sut = new ComputationThreadLauncher(queue, componentContainer, ceContainerFactory, 0, 1, TimeUnit.MILLISECONDS);
  ...
  public void call_findAndBook_when_executing_task_immediately() throws Exception {
    sut = new ComputationThreadLauncher(queue, componentContainer, ceContainerFactory, 1, 1, TimeUnit.HOURS);
    sut.start();
