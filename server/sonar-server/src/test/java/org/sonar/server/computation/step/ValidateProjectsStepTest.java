...
public class ValidateProjectsStepTest {
  ...
  public void setUp() throws Exception {
    ...
    sut = new ValidateProjectsStep(dbClient, settings);
  }
  ...
  public void not_fail_if_provisioning_enforced_and_project_exists() throws Exception {
    ...
    dbClient.componentDao().insert(dbSession, ComponentTesting.newProjectDto().setKey(PROJECT_KEY));
    dbSession.commit();
  ...
  @Test
  @Ignore
  public void fail_if_module_key_already_used_as_project_key() throws Exception {
    thrown.expect(IllegalArgumentException.class);
    thrown.expectMessage("Validation of project failed:\n" +
      "  o \"The project '"+ MODULE_KEY +"' is already defined in SonarQube but not as a module of project '" + PROJECT_KEY + "'. " +
      "If you really want to stop directly analysing project '"+ MODULE_KEY +"', please first delete it from SonarQube and then relaunch the analysis of project '" + PROJECT_KEY +" '.");

    ComponentDto project = ComponentTesting.newProjectDto().setKey(MODULE_KEY);
